...
// When changing the shipping address due to no shipping rates being available, head straight to the checkout confirmation page
    ...
    public function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)OSCOM_APP_PAYPAL_EC_ZONE > 0) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from :table_zones_to_geo_zones where geo_zone_id = '" . (int)OSCOM_APP_PAYPAL_EC_ZONE . "' and zone_country_id = '" . (int)$order->delivery['country']['id'] . "' order by zone_id");
