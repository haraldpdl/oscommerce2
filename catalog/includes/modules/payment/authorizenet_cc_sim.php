  ...
  class authorizenet_cc_sim {
    ...
    public function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_AUTHORIZENET_CC_SIM_ZONE > 0) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from :table_zones_to_geo_zones where geo_zone_id = '" . (int)MODULE_PAYMENT_AUTHORIZENET_CC_SIM_ZONE . "' and zone_country_id = '" . (int)$order->billing['country']['id'] . "' order by zone_id");
      ...
      if (!defined('MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TRANSACTION_ORDER_STATUS_ID')) {
        ...
        if (tep_db_num_rows($check_query) < 1) {
          ...
          foreach ($languages as $lang) {
            tep_db_query("insert into :table_orders_status (orders_status_id, language_id, orders_status_name) values ('" . (int)$status_id . "', '" . (int)$lang['id'] . "', 'Authorize.net [Transactions]')");
          }
