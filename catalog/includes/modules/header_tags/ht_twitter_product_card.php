  ...
  class ht_twitter_product_card {
    ...
    public function execute() {
      ...
      if ( ($PHP_SELF == 'product_info.php') && isset($_GET['products_id']) ) {
        $product_info_query = tep_db_query("select p.products_id, pd.products_name, pd.products_description, p.products_image from :table_products p, :table_products_description pd where p.products_id = '" . (int)$_GET['products_id'] . "' and p.products_status = '1' and p.products_id = pd.products_id and pd.language_id = '" . (int)$_SESSION['languages_id'] . "'");
